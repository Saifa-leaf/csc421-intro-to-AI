...
# Loop                   clauses.append(expr('On(x,z) & On(z,w) & Base (w,y) ==> Base(x,y)'))
clauses.append(expr('OnPlate(x) & OnPlate(y) & AtLeft(x,y) ==> Base_at_right(x,y)'))
clauses.append(expr('OnPlate(x) & OnPlate(y) & OnPlate(w) & AtLeft(x,w) & AtLeft(w,y) ==> Base_at_right(x,y)'))
# loop               # clauses.append(expr('OnPlate(x) & OnPlate(y) & OnPlate(w) & OnPlate(z) & AtLeft(x,w) & AtLeft(w,z) & Atleft(z,y) ==> Base_at_right(x,y)'))
clauses.append(expr('Base_at_right(x, y) ==> Object_at_right(y,x)'))
clauses.append(expr('Object_at_right(x,y) & Base(z,x) ==> Object_at_right(z,y)'))
...
clauses.append(expr('AtLeft(C, E)'))
...
kb = FolKB(clauses)
# print(kb.ask(expr('On(B, C)')))

print('Is piece B on top of piece C?')
brick = bool(len(list(fol_fc_ask(kb, expr('On(B, C)')))))
print(brick)

print('What is the type and color of the piece on top of C?')
ans = list(fol_fc_ask(kb,expr('On(a, C)')))[0][a]
co = 'Color(' + str(ans) + ', b)'
c = list(fol_fc_ask(kb,expr(co)))
ty = 'Type(' + str(ans) + ', b)'
t = list(fol_fc_ask(kb,expr(ty)))
print(ans)
print(c)
print(t)

print('What is the type of the base of H?')
ans = list(fol_fc_ask(kb,expr('Base(H, a)')))[0][a]
ty = 'Type(' + str(ans) + ', b)'
t = list(fol_fc_ask(kb,expr(ty)))
print(t)

print('What are the bricks that are right of C?')
ans = list(fol_fc_ask(kb,expr('Base(C, a)')))[0][a]
bc = 'Object_at_right(b ,' + str(ans) + ')'
b = list(fol_fc_ask(kb,expr(bc)))
print(b)

# print(list(fol_fc_ask(kb,expr('Object_at_right(a, C)'))))

print('What are all the bricks that are on top of i ?')
more = 'On(a, I)'
while ( bool(len(list(fol_fc_ask(kb,expr(more)))))):
    ans = (list(fol_fc_ask(kb,expr(more))))
    print(ans[0][a])
    more = 'On(a,' + str(ans[0][a]) + ')'